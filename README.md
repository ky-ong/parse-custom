...
if (device.platform.toLowerCase() === 'android') {
    androidParsePush(ParsePlugin);
} else if (device.platform.toLowerCase() === 'ios') {
    iosParsePush(ParsePlugin);
}
function androidParsePush(parseInstance) {
    parseInstance.register(...)
}
function iosParsePush(parseInstance) {
    parseInstance.initialize(...)
}
```

**API**

* **register**( options, successCB, errorCB )   -- only for Android
* **initialize**( appID, clientKey, successCB, errorCB )   -- only for iOS
...
```
cordova plugin add <repo> --variable --variable APP_ID=<appID> --variable CLIENT_KEY=<clientKey>
```
...
the app will be automatically invoked, and this plugin's `ParsePluginReceiver` runs before the `Activity` class or any javascript code
...
Once the device is ready, call ```ParsePlugin.register()```. This will register the device with Parse,
you should see this reflected in your Parse control panel. Once registered, the ParsePlugin object
...
```javascript
ParsePlugin.register({eventKey:"myEventKey"}, //will trigger receivePN[pnObj.myEventKey]
function() {
...
ParsePlugin.subscribe('SampleChannel', function(msg) {
    alert('OK');
}, function(e) {
    alert('error');
});
...
Anywhere in your code, you can set a listener for notification events using the ParsePlugin object (it extends Parse.Events).
```javascript
if(window.ParsePlugin){
    ...
    //
    ParsePlugin.on('receivePN:chat', chatEventHandler);
    ParsePlugin.on('receivePN:serverMaintenance', serverMaintenanceHandler);
}
```
